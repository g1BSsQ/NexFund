import {
  ...
  async function contribute(wallet: BrowserWallet, txHash: string, admin: string, assets: Asset[], name: string, vote: string, id: number, amount: number, timeStamp: string) {
    try {
      ...
      const voteScriptCbor = applyParamsToScript(
        voteCompileCode,
        [stringToHex(id), pubkeyAdmin, stringToHex(name), stringToHex(amount), stringToHex(timeStamp)],
      );
