  ...
  const formattedContributionsByAddress: Array<[string, number, number]> = 
    Array.from(contributionsByAddress.entries())
      .map(([address, amount]) => [
        address,
        amount,
        amount / 1000000 // Convert lovelace to ADA
      ]);
  
  // Return the complete stats object
  return {
    totalAmount,
    totalAmountAda: totalAmount / 1000000, // Convert lovelace to ADA
    uniqueContributors: contributorCount,
    totalContributions: contributionCount,
    contributionsByAddress: formattedContributionsByAddress,
    addressAmountPairs
  };
}